...
class DSTANet(nn.Module):
    def __init__(self, num_class=60, num_point=22, num_frame=64, num_subset=3, dropout=0., config=None, num_person=2,
    ...
    def forward(self, x, b, c):
        ...
        # NM, C, T, V
        print('output====', x.size())
        # print('att output', x.size())
        x = x.view(N, 1, self.out_channels, -1)
...
if __name__ == '__main__':
    ...
    # print(config[0][0])
    ske = torch.rand([20, 64, 22, 3])  # .cuda()
    print(net(ske, ske, ske).shape)

    # summary(net, [(3, 64, 22, 1)])
