...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, channel, joint_num, time_len, domain):
        ...
        if domain == "temporal":
            ...
            pe = utils.positionalencoding1d(joint_num * self.channel, time_len).reshape(self.channel, time_len, joint_num).unsqueeze(0).float()
            # print('pe_out', pe.shape)
            self.register_buffer('pe', pe)
    ...
    def forward(self, x):  # nctv
        # print('pe', x.shape)
        # print('pe', x.shape, self.pe.size(), self.pe[:, :, :x.size(2)].size())
...
class MT_Net(nn.Module):
    ...
    def forward(self, data_x, data_y):
        ...
        return self.relu(x), self.relu(y) #x, y
...
class Dylan_MT_Net(nn.Module):
    def __init__(self, in_channels, out_channels, num_class, num_node=22, num_frame=64, n_layers=2, attn_heads=4, dropout=0.05, l_dropout=0.2):
        ...
        self.linear1 = nn.Sequential(
            d1D(self.in_channels*self.num_node*num_frame//2, 512*mid_layer), #self.in_channels*self.num_node*num_frame//2
            nn.Dropout(l_dropout)
        )
